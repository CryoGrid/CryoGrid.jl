...
function run_solver_benchmark(solvers, grids, dts, tspan, upperbc;
    ...
    for (i, (dt, grid)) in enumerate(zip(dts,grids))
        ...
        model = Models.SoilHeat(:H, upperbc, profile; grid=grid, freezecurve=freezecurve)
        p = copy(model.pproto)
        p .= params
...
Lin  = TimeSeriesForcing(ustrip.(forcings.data.Lin), forcings.timestamps, :Lin);
Sin  = TimeSeriesForcing(ustrip.(forcings.data.Sin), forcings.timestamps, :Sin);
