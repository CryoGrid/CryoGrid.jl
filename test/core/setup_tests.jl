...
@testset "Setup" begin
    @testset "3-layer" begin
        ...
        function checkfields()
            ...
            @test hasproperty(setup.state,:top)
            @test hasproperty(setup.state,:testground)
            @test hasproperty(setup.state,:bottom)
        end
        ...
        @test hasproperty(setup.state.testground,:k)
        ...
        setup = CryoGridSetup(strat,grid)
        checkfields()
        @test hasproperty(setup.state.testground,:w)
        ...
        setup = CryoGridSetup(strat,grid)
        checkfields()
        @test hasproperty(setup.state.testground,:a)
        setup.state.testground.a[1] = 2.0
        @test setup.state.testground.a[1] == 2.0
    ...
    @testset "4-layer" begin
        ...
        @test hasproperty(setup.uproto,:bottom)
        @test hasproperty(setup.state,:top)
        @test hasproperty(setup.state,:testground1)
        @test hasproperty(setup.state,:testground2)
        @test hasproperty(setup.state,:bottom)
        # check for variables
        @test hasproperty(setup.uproto.testground1, :x)
        @test hasproperty(setup.state.testground1, :k)
        @test hasproperty(setup.state.testground1, :w)
        @test hasproperty(setup.uproto.testground2, :x)
        @test hasproperty(setup.state.testground2, :k)
        @test hasproperty(setup.state.testground2, :w)
        # clean-up method definitions (necessary for re-running test set)
        Base.delete_method(@which CryoGrid.variables(TestGroundLayer(),TestGroundProcess()))
        Base.delete_method(@which CryoGrid.variables(TestGroundLayer()))
    end
