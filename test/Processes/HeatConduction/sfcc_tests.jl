...
@testset "SFCC" begin
    @testset "McKenzie freeze curve" begin
        @testset "Sanity checks" begin
            f = McKenzie()
            θsat = 0.8
            @test isapprox(f(-10.0,1,θsat,θsat), 0.0, atol=1e-6)
            @test f(0.0,1,θsat,θsat) ≈ θsat
            θl = f(-0.1,1,θsat,θsat)
            @test θl > 0.0 && θl < 1.0
        end
    ...
    @testset "Westermann freeze curve" begin
        @testset "Sanity checks" begin
            ...
            θtot = 0.8
            δ = 0.1
            @test isapprox(f(-10.0,δ,θtot), 0.0, atol=1e-2)
            @test f(0.0,δ,θtot) ≈ θtot
            θl = f(-0.1,δ,θtot)
            @test θl > 0.0 && θl < 1.0
        end
        @testset "Newton solver checks" begin
    ...
    @testset "Van Genuchten freeze curve" begin
        @testset "Sanity checks" begin
            ...
            n = 2.0
            L = Heat{:H}().params.L
            @test isapprox(f(-10.0,α,n,0.0,θsat,θsat,L), 0.0, atol=1e-6)
            @test f(0.0,α,n,0.0,θsat,θsat,L) ≈ θsat
            θl = f(-0.1,α,n,0.0,θsat,θsat,L)
            @test θl > 0.0 && θl < 1.0
        end
        @testset "Newton solver checks" begin
            tol = 0.01
