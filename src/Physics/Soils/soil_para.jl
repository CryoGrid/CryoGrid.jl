"""
    CharacteristicFractions{Tpor,Tsat,Torg} <: SoilParameterization
...
"""
Base.@kwdef struct CharacteristicFractions{Tpor,Tsat,Torg,Txic} <: SoilParameterization
...
saturation(soil::Soil{<:CharacteristicFractions}, state=nothing) = soil.para.sat
porosity(soil::Soil{<:CharacteristicFractions}, state=nothing) = soilcomponent(Val{:θp}(), soil.para)
mineral(soil::Soil{<:CharacteristicFractions}, state=nothing) = soilcomponent(Val{:θm}(), soil.para)
organic(soil::Soil{<:CharacteristicFractions}, state=nothing) = soilcomponent(Val{:θo}(), soil.para)

CryoGrid.parameterize(para::CharacteristicFractions) = CharacteristicFractions(
...
CryoGrid.variables(soil::Soil{<:CharacteristicFractions}) = CryoGrid.variables(soil, processes(soil))

# Helper functions for obtaining soil compositions from characteristic fractions.
soilcomponent(::Val{var}, para::CharacteristicFractions) where var = soilcomponent(Val{var}(), para.por, para.sat, para.org, para.xic)
...
Constructs a `CharacteristicFractions` soil parameterization from the component volumetric fractions of
...
function from_components(θo, por, sat=1.0, xic=0.0)
    ...
    θm = 1 - max(por, xic) - θo
    org = θo / (θm + θo)
    return CharacteristicFractions(;por, sat, org, xic)
end

"""
    Soil(
        para::CharacteristicFractions=CharacteristicFractions();
        kwargs...
    )
