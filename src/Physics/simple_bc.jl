...
struct ConstantBC{P,S,T} <: BoundaryProcess{P}
    value::T
    ConstantBC(::Type{P}, ::Type{S}, value::T) where {P<:SubSurfaceProcess,S<:BoundaryCondition,T} = new{P,S,T}(value)
end
...
CryoGrid.BoundaryCondition(::Type{<:ConstantBC{P,S}}) where {P,S} = S()
...
struct PeriodicBC{P,S,T1,T2,T3,T4} <: BoundaryProcess{P}
    ...
    PeriodicBC(::Type{P}, ::Type{S}, period::T1=1.0, amplitude::T2=1.0, phaseshift::T3=0.0, level::T4=0.0) where
        {P<:SubSurfaceProcess,S<:BoundaryCondition,T1,T2,T3,T4} =
...
CryoGrid.BoundaryCondition(::Type{<:PeriodicBC{P,S}}) where {P,S} = S()
