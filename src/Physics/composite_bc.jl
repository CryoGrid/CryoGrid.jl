...
struct CompositeBoundaryProcess{B1,B2,P,F,S} <: BoundaryProcess{P}
    ...
    function CompositeBoundaryProcess(op::F, bc1::B1, bc2::B2) where {F,P,B1<:BoundaryProcess{P},B2<:BoundaryProcess{P}}
        @assert BoundaryCondition(bc1) == BoundaryCondition(bc2) "boundary condition styles (e.g. Dirichlet vs Neumann) must match"
        new{B1,B2,P,F,typeof(BoundaryCondition(bc1))}(op,bc1,bc2)
    end
end
...
CryoGrid.BoundaryCondition(::Type{CompositeBoundaryProcess{B1,B2,P,F,S}}) where {F,P,B1,B2,S} = S()
