...
"""
struct JsonSpec{Version} <: ForcingInputFormat end

export ForcingInputFormat, JsonSpec
...
Loads forcing data from the given file according to the format specified by `spec::ForcingInputFormat`. Default is JsonSpec{1}.
...
"""
loadforcings(filename::String, units...; spec::Type{T}=JsonSpec{1}) where {T <: ForcingInputFormat} = loadforcings(T, filename, units...)
loadforcings(resource::Resource, units...; spec::Type{T}=JsonSpec{1}, outdir=DEFAULT_FORCINGS_DIR) where {T <: ForcingInputFormat} = loadforcings(T, fetch(resource, outdir), units...)
