...
using CryoGrid.Common.Math: nonlineardiffusion!
using CryoGrid.Layers: Soil, SoilParams
using CryoGrid.Processes: Constant
using CryoGrid.Setup: CryoGridSetup
...
"""
boundaryflux(boundary::Boundary, bc::B, sub::SubSurface, h::Heat, sbound, ssub) where {B<:BoundaryProcess{<:Heat}} =
    boundaryflux(CryoGrid.BoundaryStyle(B), boundary, bc, sub, h, sbound, ssub)
...
"""
function CryoGrid.interact!(top::Top, bc::B, sub::SubSurface, heat::Heat, stop, ssub) where {B<:BoundaryProcess{<:Heat}}
...
"""
function CryoGrid.interact!(sub::SubSurface, heat::Heat, bot::Bottom, bc::B, ssub, sbot) where {B<:BoundaryProcess{<:Heat}}
...
"""
function CryoGrid.interact!(::SubSurface, ::Heat, ::SubSurface, ::Heat, s1, s2)
...
CryoGrid.variables(::Soil, ::Heat, ::FreezeCurve) = ()
...
CryoGrid.JacobianStyle(::Type{<:HeatOnlySetup}) = TridiagJac()
