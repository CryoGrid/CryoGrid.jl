...
struct Prognostic{name,T,S} <: Var{name,T,S}
    ...
    Prognostic(name::Symbol, ::Type{T}, dims::OnGrid) where {T} = error("Off-cell prognostic/algebraic spatial variables are not currently supported.")
end
struct Algebraic{name,T,S} <: Var{name,T,S}
    ...
    Algebraic(name::Symbol, ::Type{T}, dims::OnGrid) where {T} = error("Off-cell prognostic/algebraic spatial variables are not currently supported.")
end
