...
soilprofile = SoilProfile(
	...
	1000.0u"m" => (0.80,0.0,0.05,0.15,0.80),
...
C(g::Plots.ColorGradient) = reshape(Plots.RGB[g[z] for z=LinRange(0,1,length(zs))],(1,:))
plot(ustrip(t), T[zs,:]'.-273.15, label=Hgrid[zs]', xlabel="Days", ylabel="Temperature (°C)", c=Plots.cgrad(:berlin,rev=true) |> C, leg=false, cbar=true)
...
plot(sol.t, Tout[zs,:]', legend=nothing)
matwrite("/tmp/cryogrid_Ts_365d_highacc.mat", Dict("data"=>Tout))
