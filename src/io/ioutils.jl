...
# Recursive algorithm for converting a dictionary into type stable nested named tuples
tonamedtuple(d::AbstractDict{Symbol}) = NamedTuple{Tuple(keys(d))}(d |> values .|> tonamedtuple |> Tuple)
tonamedtuple(vals::AbstractArray) = Array(replace(nothing=>missing, vals))
tonamedtuple(val) = val
